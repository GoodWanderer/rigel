(function($) { "use strict";
	...
	if(fixed_enabled){
		jQuery(window).scroll(function(){
			if(jQuery(window).scrollTop() > aboveHeight ){
				...
				jQuery(".fixedp").css({"display":"block"});

			}else{
				jQuery("#header").css({"top":"auto"}).removeClass("fixed-nav");
                jQuery(".fixedp").css({"display":"none"});
			}
		});
	}else {
	...
	jQuery(".protfolio-filter2 ul").each(function() {
		var $this = jQuery(this);
		$this.find("li a").click(function() {
			$this.find("li").removeClass("current");
		    jQuery(this).parent().addClass("current");
	...
	jQuery(window).load(function() {
		if (jQuery(".protfolio-filter2").length > 0) {
		   jQuery(".protfolio-filter2 u li:first-child a").click();
		}
	});

jQuery(".protfolio-filter2 ul li a").click(function(){
    //if(!jQuery(this).parent().hasClass('current')) {
    var selector = jQuery(this).attr("data-filter");
	jQuery(".portfolio-0").each(function() {
		var $this = jQuery(this);
        $this.hide();
	});
